...
{int32_t *in1,*in2,*in3,*meas,*c1,*c2,k;
 int64_t tmp1,tmp2;
 if(!xQueueReceive(qh1, &in1, portMAX_DELAY)) uart_puts("rx1 failed\n\0");
 if(!xQueueReceive(qh1, &in2, portMAX_DELAY)) uart_puts("rx2 failed\n\0");
 if(!xQueueReceive(qh1, &in3, portMAX_DELAY)) uart_puts("rx3 failed\n\0");
...
{char c[20];
 ...
 uart_puts("f2\0");
 if (in[2*N]>0)
    if(xQueueSend(qh1, &in, portMAX_DELAY)!= pdPASS) uart_puts("tx failed\n\0");
